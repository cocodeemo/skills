...
- **辩论跑完后必须 `wait` 到底，不要 poll**：`terminal(background=True)` + `notify_on_complete=True` 有时不触发通知，导致用户发「跑完了吗」时才去查。正确做法：`background=True` 启动后，用 `process(action='wait', timeout=300)` 阻塞等待，跑完直接出报告。不要中途 `poll` 打断。用户明确说「不用提前结束，还在跑就继续等，不要每次还要我问才能给我报告」——`background` 模式的通知不可靠，`wait` 才是稳定方案。
