...
```bash
python3 /mnt/d/debate-engine/stock_debate.py 688270   # 臻镭科技（A股科创板）
python3 /mnt/d/debate-engine/stock_debate.py 000762   # 西藏矿业（A股深市）
python3 /mnt/d/debate-engine/stock_debate.py 00700    # 腾讯控股（港股，自动识别）
python3 /mnt/d/debate-engine/stock_debate.py 688270 --rounds 3  # 3轮辩论
python3 /mnt/d/debate-engine/stock_debate.py 688270 --model deepseek-v4-pro  # 指定模型
```
...
- **数据三方交叉验证**：仅靠腾讯一个数据源不可靠（52周高低不除权就是教训）。v1.9.1+ 增加了 EastMoney API（`push2.eastmoney.com`）作为第三数据源：PE 三方对比（腾讯/东财/计算值），市值双方对比。偏差>15%（PE）/ >10%（市值）→ 报警。注意：EastMoney 财务数据是年度口径，AKShare 是季度口径，不能直接对比营收和净利——只交叉验证实时指标。
...
- **Git 操作铁律**：禁止未经用户许可执行 `git push`。任何 GitHub 修改（push、skill 更新、prompt 修改）都必须先获得同意。用户说推才能推。本 session 因擅自 push+改 prompt 被要求回滚。
